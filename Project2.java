import java.math.BigInteger;

public class Project2 {
    
    // Returns number of bits in number in hex
    static String getNumBits(BigInteger number) {
        int num = number.bitLength();
        return Integer.toHexString(Integer.valueOf(num));

    }

    // turn BigInteger into hex
    static String getHexValue(BigInteger number) {
        return Double.toHexString(number.doubleValue());
    }

    // returns private key in hex
    static String getPrivateKey(BigInteger e, BigInteger p, BigInteger q) {
        BigInteger one = new BigInteger("1");
        BigInteger p_minus_one = p.subtract(one);
        BigInteger q_minus_one = q.subtract(one);
        BigInteger z = p_minus_one.multiply(q_minus_one);

        BigInteger mult_inverse = z.modInverse(e); 

        // should the below be an integer or double or long?
        return Integer.toHexString(Integer.valueOf(mult_inverse.intValueExact()));
    }

    // why are the directions telling me to look at modInverse
    // encrypt message
    static String encrypt(int m, BigInteger e, BigInteger N) {
        BigInteger bigInt_m = new BigInteger("" + m);
        BigInteger encrypted = bigInt_m.modPow(e, N);
        return Integer.toHexString(encrypted.intValue());
    }


    static String decrypt(long message, BigInteger d, BigInteger N) {
        BigInteger bigInt_message = new BigInteger("" + message);
        BigInteger decrypted = bigInt_message.modPow(d, N);
        return Integer.toHexString(decrypted.intValue());
    }

    public static void main(String[] args) {
        // Set up
        BigInteger N = new BigInteger("1180590145325084590940239490622584768749437722435980772180699");
        BigInteger e = new BigInteger("65537");
        BigInteger p = new BigInteger("", 16);
        BigInteger q = new BigInteger("", 16);

        // Question 1
        System.out.println("N has " + getNumBits(N) + " bits.");

        // used sage Math for part 1 of question 2
        // Question 2 part 2
        System.out.println("p has " + getNumBits(p) + " bits.");
        System.out.println("q has " + getNumBits(q) + " bits.");

        // Question 3
        System.out.println("Bob's private key has a value of " + getPrivateKey(e, p, q));

        // Question 4
        String encrypted = encrypt(3, e, N);
        System.out.println("If Alice's message, m = 3, were encrpted it would be " + encrypted);

        long message = Long.parseLong(encrypted, 16);
        // Question 5
        System.out.println("Alice's decrypted message would then be " + decrypt(message, e, N));
        


    }

}
